...
| Order/fill audit trail | `order_audit_events` | Exists, but **a real, previously-undiscovered bug was found and fixed** (not yet deployed): its `event` check constraint only allowed `('open','close','reject')`, so every `modify` (SL/TP change) audit insert has been silently rejected and swallowed since that feature shipped. Pending-order actions were never audited at all. Fix is in `order-position-id-integrity.sql`, undeployed as of this audit. |
